...
void Wireless_Send(DATA_MK imssage)
/*
data为整型数据（）
...
*/
{
	uint8_t waveform[4 + UserDataLen] = {0};//数据帧缓存
	uint8_t _cnt = 0;
	...
	int16_t _UserData_1 = (int16_t)(imssage.temperature_one * 10);
	int16_t _UserData_2 = (int16_t)(imssage.window_ZT_zero);
	int16_t _UserData_3 = (int16_t)(imssage.humidity_zero);
	int16_t _UserData_4 = (int16_t)(imssage.chuanglian_ZT_zero);
	int16_t _UserData_5 = (int16_t)(imssage.fan_ZT_zero);
	int16_t _UserData_6 = (int16_t)(imssage.led_ZT_zero);
	//数据区使用小端模式, 低字节在前。
	/*-----------------------此处为待写入数据位置 ---------------------------*/
	//温度float  .*
	...
	//窗户状态 bool 0 or 1
	waveform[_cnt++] = BYTE0(_UserData_2);   	// 数据内容
    waveform[_cnt++] = BYTE1(_UserData_2);
	//湿度
	waveform[_cnt++] = BYTE0(_UserData_3);   	// 数据内容
    waveform[_cnt++] = BYTE1(_UserData_3);
	//窗帘
	waveform[_cnt++] = BYTE0(_UserData_4);   	// 数据内容
    waveform[_cnt++] = BYTE1(_UserData_4);
	//风扇
	waveform[_cnt++] = BYTE0(_UserData_5);   	// 数据内容
    waveform[_cnt++] = BYTE1(_UserData_5);
	//led
	waveform[_cnt++] = BYTE0(_UserData_6);   	// 数据内容
    waveform[_cnt++] = BYTE1(_UserData_6);
	
	
	waveform[2] = _cnt - 3; 					// 写入有效数据字节数
	
	uint8_t _sumcheck = 0;  			 			// 和校验SC
    uint8_t _addcheck = 0;   					// 附加校验AC
	for(uint8_t i = 0; i < waveform[2] + 3; i++) {
...
# define TFMINI_DATA_Len UserDataLen+5//数据帧长度
...
//串口接收函数
{
	...
	int j = 0;
	 ...
	 if (g_TFminiPlus_num == 0 && data == TFMINT_DATA_HEAD) {                      // 校验并记录帧头
		...
		TFminiPlus_buf[2] = data;
		Mode_store = data;
		g_TFminiPlus_num = 3;
	} else if (g_TFminiPlus_num == 3){
		TFminiPlus_buf[3] = data;
		g_TFminiPlus_num = 4;
	} else if (g_TFminiPlus_num >= 4 && g_TFminiPlus_num <= (TFMINI_DATA_Len - 1)) {
        TFminiPlus_buf[g_TFminiPlus_num] = data;
        g_TFminiPlus_num++;
    } else if (g_TFminiPlus_num == (TFMINI_DATA_Len)) {
		...
		if (checksum == data) {                                                 // 校验Checksum
			...
			while ( j < UserDataLen )
			{
				g_TFmin_Dist |= TFminiPlus_buf[5+j] << (8*j);
				j++;
			}
            g_TFminiPlus_flag = 1;
        } else {
