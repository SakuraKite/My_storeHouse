...
//-------------------------------------------------------------------------------------------------------	
	...
	if (get_G_2 != get_G_5)	find_data = -1;
	else if ((get_G_1 != get_G_6) && (X_now_pos == 2 || X_now_pos == -2)) find_data = -2;
	else 					find_data = 1;
	
	if (Close_XJ_time == 0 && Close_XJ_timee == 0 && ZT_Flag != houtui)
	{
		if (find_data >= 0)
		{
			if (get_G_2 == get_G_5)
			{
				...
				else if (get_G_3 == BLACK) {PWM_L = 30; PWM_R = 45;}
				else 		{PWM_L = 45; PWM_R = 30;}
			}
		...
		else 
		{
			if (find_data == -1)
			{
				if (get_G_2 == BLACK)
				{
					if (get_G_3 == BLACK)	{PWM_L = 30; PWM_R = 35;}
					else	{xiu_flag = 1;PWM_L = 10; PWM_R = 35;}
				}
				else if (get_G_5 == BLACK)	
				{
					if (get_G_4 == BLACK)	{PWM_L = 35; PWM_R = 30;}
					else	{xiu_flag = 1;PWM_L = 35; PWM_R = 10;}
				}
				else
				{
					if (get_G_4 == BLACK)	{xiu_flag = 1;PWM_L = 45; PWM_R = 10;}
					else if (get_G_3 == BLACK)	{xiu_flag = 1;PWM_L = 10; PWM_R = 45;}
					else {PWM_L = qianjin_PWM_Left; PWM_R = qianjin_PWM_Right;}
				}
			}
			else
			{
					if (get_G_2 == BLACK)
					{
						if (get_G_3 == BLACK)	{PWM_L = 30; PWM_R = 40;}
						else	{xiu_flag = 1;PWM_L = 10; PWM_R = 35;}
					}
					else if (get_G_5 == BLACK)	
					{
						if (get_G_4 == BLACK)	{PWM_L = 45; PWM_R = 30;}
						else	{xiu_flag = 1;PWM_L = 45; PWM_R = 10;}
					}
					else
					{
						if (get_G_4 == BLACK)	{xiu_flag = 1;PWM_L = 50; PWM_R = 10;}
						else if (get_G_3 == BLACK)	{xiu_flag = 1;PWM_L = 10; PWM_R = 55;}
						else if (get_G_1 == BLACK) {xiu_flag = 1;PWM_L = 10; PWM_R = 40;}
						else if (get_G_6 == BLACK)	{xiu_flag = 1; PWM_L = 40;PWM_R = 10;}
						else {PWM_L = qianjin_PWM_Left; PWM_R = qianjin_PWM_Right;}
					}
			}
		}
...
void Encounter_Road(void)
{
				...
				if(Y_now_pos == 4 || Y_now_pos == 5 || Y_now_pos == 6 || (Y_now_pos == 7 && X_now_pos != 1))	X_now_pos++;
				  ...
				  if ((Y_now_pos == 7 || Y_now_pos == 4) && (X_now_pos > 0))
					time_fuzhi(5);
					  ...
					  time_fuzhi(5);
				  else if  ((Y_now_pos == 5) && (X_now_pos < 0))
					time_fuzhi(6);
				  else time_fuzhi(2);
			  }
		  }
	...
	if (Road_time == 0 && Road_timee == 0 && Road_timeer == 0 && Road_timeeeer == 0 && Road_timeeerr == 0)
	{
		...
		else if(X_now_pos == -1 && Y_now_pos<4 && Y_now_pos >= 2 )
		{
			EXTI_InitStructure.EXTI_LineCmd = ENABLE;
			EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
			EXTI_InitStructure.EXTI_Line=EXTI_Line1;
		...
		else if (Y_now_pos == 7 && Road_timeeeer == 0 && X_now_pos == 1)
		{
			EXTI_InitStructure.EXTI_LineCmd = ENABLE;
			EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
			EXTI_InitStructure.EXTI_Line=EXTI_Line1;
		...
		else if (Y_now_pos == 1 && Road_timeeeer == 0 && X_now_pos == -1)
		{
			EXTI_InitStructure.EXTI_LineCmd = ENABLE;
			EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
			EXTI_InitStructure.EXTI_Line=EXTI_Line1;
